...
import { ShieldAlert, Lock, RefreshCw, AlertTriangle, Bot, Wrench, Settings, Loader2 } from 'lucide-react';
...
const MaintenanceMode: React.FC = () => {
  ...
  // --- REDIRECCIÓN AUTOMÁTICA DE ADMIN (FORCE EJECT) ---
  useEffect(() => {
      if (!loading && user?.isAdmin) {
          // Si es admin, lo sacamos de aquí inmediatamente
          navigate('/home', { replace: true });
      }
  }, [user, loading, navigate]);

  // --- SEGURIDAD VISUAL ---
  // Si ya detectamos que es admin, NO renderizamos nada (pantalla blanca)
  // mientras el useEffect de arriba hace la redirección.
  // Esto evita el "flash" rojo.
