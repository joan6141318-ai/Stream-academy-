...
const AdminDashboard: React.FC = () => {
  ...
  // --- OPTIMIZACIÓN CRÍTICA: useMemo para filtrado ---
  ...
  const handleUpdateAgencyCode = async () => {
      ...
      try {
          ...
          setNewAgencyCode('');
      } catch(e) { alert("Error"); } finally { setIsUpdatingKey(false); }
  };
  ...
  return (
    <div className="flex flex-col h-full w-full bg-gray-50 dark:bg-black transition-colors duration-300">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide p-4 pb-24">
        {activeTab === 'users' && (
            <div className="space-y-4 animate-slide-up">
                <div className="bg-white dark:bg-brand-dark-card p-2 rounded-2xl shadow-sm border border-gray-100 dark:border-white/5 flex items-center sticky top-0 z-20">
                    ...
                    <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Filtrar..." className="bg-transparent border-none text-sm font-bold w-full ml-3 focus:outline-none dark:text-white placeholder-gray-300" />
                </div>
                <div className="grid gap-3">
                    {isLoading ? <div className="text-center py-10">Cargando...</div> : filteredUsers.map((u) => (
        ...
        {/* Simplified Security Tab for brevity - Logic kept */}
        ...
        {activeTab === 'security' && securityView === 'lockdown' && (
            <div className="space-y-4 animate-slide-up">
                ...
                <div className={`p-6 rounded-2xl text-center ${currentMode === 'lockdown' ? 'bg-red-700 text-white' : 'bg-white dark:bg-brand-dark-card border'}`}>
                    ...
                    <Button onClick={() => handleSetMode('lockdown')} variant="black" className="w-full">{currentMode === 'lockdown' ? 'DESACTIVAR' : 'ACTIVAR LOCKDOWN'}</Button>
                </div>
                <div className={`p-6 rounded-2xl text-center ${currentMode === 'maintenance' ? 'bg-purple-700 text-white' : 'bg-white dark:bg-brand-dark-card border'}`}>
                    ...
                    <Button onClick={() => handleSetMode('maintenance')} variant="black" className="w-full">{currentMode === 'maintenance' ? 'FINALIZAR' : 'INICIAR MANTENIMIENTO'}</Button>
                </div>
            </div>
        )}

        {/* Keeping other tabs simplified for XML length but functional */}
            ...
            <div className="space-y-6 animate-slide-up">
                ...
                {pkView === 'assign' ? (
                     <div className="bg-white dark:bg-brand-dark-card p-4 rounded-3xl border border-gray-100 dark:border-white/5 space-y-3">
                         ...
                         {localSchedule.potential.map((ev, i) => (<div key={i} className="flex gap-2"><input value={ev.id1} onChange={e => handleScheduleChange('potential', i, 'id1', e.target.value)} className="w-full p-2 bg-gray-50 rounded border text-center font-bold text-xs" placeholder="ID 1" /><span className="self-center font-black text-xs">VS</span><input value={ev.id2} onChange={e => handleScheduleChange('potential', i, 'id2', e.target.value)} className="w-full p-2 bg-gray-50 rounded border text-center font-bold text-xs" placeholder="ID 2" /></div>))}
                         <Button onClick={saveSchedule} fullWidth className="mt-4"><Save size={14} className="mr-2"/> Guardar</Button>
                     </div>
                ) : (
                    <div className="space-y-3">{pendingRequests.map(req => (<div key={req.id} className="bg-white p-4 rounded-xl shadow-sm border flex justify-between items-center"><div><h3 className="font-black">{req.bigoId}</h3><p className="text-xs text-gray-400">{req.date}</p></div><div className="flex gap-2"><button onClick={() => updatePKRequestStatus(req.id, 'rejected')} className="p-2 bg-red-50 text-red-500 rounded"><X size={16}/></button><button onClick={() => updatePKRequestStatus(req.id, 'approved')} className="p-2 bg-green-50 text-green-500 rounded"><Check size={16}/></button></div></div>))}</div>
                )}
            </div>
        )}
        ...
        {activeTab === 'comms' && (
             <div className="bg-white dark:bg-brand-dark-card p-6 rounded-3xl shadow-lg border border-gray-100 dark:border-white/5 space-y-4">
                 <input type="password" value={restApiKey} onChange={e => setRestApiKey(e.target.value)} placeholder="OneSignal API Key" className="w-full p-3 bg-gray-50 rounded-lg text-xs font-mono border" />
                 <textarea value={alertMessage} onChange={e => setAlertMessage(e.target.value)} placeholder="Mensaje push..." className="w-full p-3 bg-gray-50 rounded-lg text-sm border h-24" />
