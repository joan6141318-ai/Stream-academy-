...
const GameRunTool: React.FC = () => {
  ...
  // Carga de imagen segura (inicializada como null)
  const astronautImg = useRef<HTMLImageElement | null>(null);
  
  useEffect(() => {
      // Configuración de imagen segura dentro del efecto
      const img = new Image();
  ...
  // Audio - Robust implementation
  const playSound = (type: 'jump' | 'collect' | 'gameover' | 'click') => {
    if (isMuted) return;
    try {
        ...
        // Silently fail if audio is blocked to prevent game crash
  ...
  const triggerJump = (e?: any) => {
    // Universal event handler for Touch and Mouse
    if (e) {
        // Only prevent default on touchstart to allow click events but prevent scrolling/zoom
  ...
  useEffect(() => {
    ...
    let gems: { x: number; y: number; w: number; h: number; collected: boolean }[] = [];
    ...
    const drawAstronaut = (x: number, y: number) => {
        let runBobOffset = 0;
        ...
        try {
            // Check if image ref is loaded and complete
    ...
    const gameLoop = () => {
        ...
        ctx.fillStyle = '#000000'; ctx.fillRect(0,0,canvas.width,canvas.height); // Black BG
